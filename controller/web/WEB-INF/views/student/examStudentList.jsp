...
<head>
	...
	$(function() {
	    ...
	    $('#dataList').datagrid({ 
	        ...
	        fit: true,//自动大小 
	        method: "post",
	        url:"/exam/studentExamList",//"ExamServlet?method=StudentExamList&t="+new Date().getTime(),
	        idField:'id', 
	        ...
	        columns: [[  
				{field:'chk',checkbox: true,width:50},
 		        ...
 		        {field:'time',title:'考试时间',width:150, sortable: true},
	 		...
	 		toolbar: [
	        	{
	        		...
	        		handler: function(){
	                	...
	                	if(exam == null){
	                    	...
	                    	$.ajax({
	                    		type: "post",
	        					url: "/score/columnList",//"ScoreServlet?method=ColumnList",
	        					data: data,
	        					dataType: "json",
	        					async: false,
	                    	...
	                    	setTimeout(function(){
	        			    	$("#escoreList").datagrid("options").url = "/score/scoreList";//"ScoreServlet?method=ScoreList&t="+new Date().getTime();
...
/*
				{
					...
					handler: function(){
						var exam = $("#dataList").datagrid("getSelected");
						if(exam == null){
							$.messager.alert("消息提醒", "请选择考试进行考试!", "warning");
						} else {
							$.ajax({
								url:"/student/boolExam",
								data:{id: exam.id, gradeid: exam.gradeid, clazzid:exam.clazzid,courseid:exam.courseid, type: exam.type},
								type:"post",
								dataType: "json",
								async: false,
								success:function (data) {
									if(data.aBoolean=="true"){
										window.location="/examination/index?examid="+data.examid;
									}
									else{
										alert("你已参加过该考试");
									}
