	...
	$(function() {	
	    	...
	    	buttons: [
	  	    		{
	  					...
	  					handler:function(){
	  						...
	  						if(!validate){
	  							...
	  							$.ajax({
	  								type: "post",
	  								url: "/admin/editPasswod",//"SystemServlet?method=EditPasswod&t="+new Date().getTime(),
		...
		$("#systemTable a").each(function(){
			$(this).click(function(){
				...
				$.ajax({
					type: "post",
					url: "/admin/EditSystemInfo",//"SystemServlet?method=EditSystemInfo&t="+new Date().getTime(),
					data: data,
					success: function(msg){
						if(msg == "success"){
	...
	<div id="editDialog" style="padding: 20px;">
    	...
    	<table id="systemTable" class="table table-border table-bordered table-striped" style="width: 650px" cellpadding="8" >
    		...
    		<tr>
    			<td>禁止教师<br/>登录系统 </td>
				<c:choose>
					<c:when test="${forbidTeacher ==0 }">
    					<td><input id="forbid_teacher"  type="checkbox" name="forbidTeacher" /></td>
					</c:when>
					<c:otherwise>
						<td><input id="forbid_teache2" checked="true" type="checkbox" name="forbidTeacher" /></td>
					</c:otherwise>
				</c:choose>
    		...
    		<tr>
    			<td>禁止学生<br/>登录系统 </td>
				<c:choose>
					<c:when test="${forbidStudent ==0 }">
						<td><input id="forbid_student"  type="checkbox" name="forbidStudent" /></td>
					</c:when>
					<c:otherwise>
						<td><input id="forbid_studen2"  type="checkbox" checked="checked" name="forbidStudent" /></td>
					</c:otherwise>
				</c:choose>
    			<td><a data-options="iconCls:'icon-save'" class="easyui-linkbutton">保存</a></td>
    		</tr>
