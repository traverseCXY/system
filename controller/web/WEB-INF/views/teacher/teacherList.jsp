...
<head>
	...
	$(function() {	
	    ...
	    $('#dataList').datagrid({ 
	        ...
	        fit: true,//自动大小 
	        method: "post",
	        url:"/teacher/list",//"TeacherServlet?method=TeacherList&t="+new Date().getTime(),
	        idField:'id', 
        	...
        	if(selectLength == 0){
            	...
            	$.messager.confirm("消息提醒", "将删除与教师相关的所有数据，确认继续？", function(r){
            		if(r){
            			$.ajax({
							type: "post",
							url: "/teacher/deleteTeacher",//"TeacherServlet?method=DeleteTeacher",
				...
				},
	    		{
					...
					handler:function(){
						...
						if(!validate){
							...
							$.ajax({
								type: "post",
								url: "/teacher/addTeacher",//"TeacherServlet?method=AddTeacher",
								data: data,
								success: function(msg){
									if(msg == "success"){
	  ...
	  //下拉框通用属性
	  	...
	  	$("#add_courseList").combobox({
	  		url: "/course/list",//"CourseServlet?method=CourseList&t="+new Date().getTime(),
	  		onLoadSuccess: function(){
		  		//默认选择第一条数据
				var data = $(this).combobox("getData");;
				$(this).combobox("setValue", data[0].id);
				...
				},
	    		{
					...
					handler:function(){
						...
						if(!validate){
							...
							$.ajax({
								type: "post",
								url: "/teacher/editTeacher",//"TeacherServlet?method=EditTeacher",
								data: data,
								success: function(msg){
									if(msg == "success"){
...
<body>
	...
	<div id="addDialog" style="padding: 10px;">  
   		<div style=" position: absolute; margin-left: 560px; width: 250px; height: 300px; border: 1px solid #EEF4FF" id="photo">
    		<img alt="照片" style="max-width: 250px; max-height: 300px;" title="照片" src="/static/image/teacher.jpg" />
	    </div> 
	...
	<div id="editDialog" style="padding: 10px">
		<div style=" position: absolute; margin-left: 560px; width: 250px; height: 300px; border: 1px solid #EEF4FF">
	    	<img id="edit_photo" alt="照片" style="max-width: 200px; max-height: 400px;" title="照片" src="/static/image/teacher.jpg" />
	    </div>   
